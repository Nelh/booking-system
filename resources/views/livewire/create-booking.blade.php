<div class="relative">
    <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
        <form wire:submit.prevent="createBooking" class="mt-8">
            <div class="mb-6">
                <label for="service" class="inline-block text-slate-700 text-lg font-semibold mb-2">{{ __('Select service') }}</label>
            ...
            @if(config('booking-system.layout.employee-picker') == 'picker-2')
                @if($employees->count() > 0)
                    <label for="employee" class="inline-block text-slate-700 text-xl mb-2">{{ __('Select employee') }}</label>
            ...
            @else
                <div class="mb-6 {{ !$employees->count() ? 'opacity-25' : '' }}">
                    <label for="employee" class="inline-block text-slate-700 text-lg font-semibold mb-2">{{ __('Select employee') }}</label>
            ...
            <div class="{{ !$this->selectedService || !$this->selectedEmployee ? 'opacity-25' : '' }}">
                ...
                <label for="employee" class="inline-block text-lg font-semibold">{{ __('Select appointment date') }}</label>
                @endif
            ...
            @if ($this->hasDetailsToBook)
                ...
                <div class="mb-6">
                    <div class="mb-3">
                        <label for="name" class="inline-block text-slate-700 font-bold mb-2">{{ __('Your name') }}</label>
                    ...
                    <div class="mb-3">
                        <label for="email" class="inline-block text-slate-700 font-bold mb-2">{{ __('Your email') }}</label>
